...
{{#option "`--go`" }}
Generate Go types, cell codecs, and getter codecs using Acton's published generator
module. Requires a Go toolchain on `PATH`; no separate generator installation is
needed.
...
Acton runs the generator from the pinned published module
`github.com/ton-blockchain/tolk-abi-to-go/cmd/tolk-abi-to-go@v0.1.0`. Install Go
from https://go.dev/dl/ and ensure `go` is on `PATH`. Use a Go version meeting the
pinned module's `go.mod` requirement; an execution failure reports the required
version. No separate helper installation is needed: Go fetches the module itself on
first use, the same way `npx` fetches the TypeScript generator.

For each invocation, Acton runs
`go run -mod=readonly -trimpath github.com/ton-blockchain/tolk-abi-to-go/cmd/tolk-abi-to-go@v0.1.0`
from an empty temporary directory with `CGO_ENABLED=0` and `GOWORK=off`. A pinned
`pkg@version` run ignores the `go.mod` of the current directory and every parent.
Caller `GOFLAGS`, including saved Go environment defaults, are ignored for this
run; proxy, cache, and toolchain settings are retained. The temporary directory is
removed afterward, including when Go fails. The Go toolchain uses its standard
module and build caches; first use downloads the generator and its pinned
dependencies, including `tonutils-go`. Offline use requires those dependencies and
a compatible Go toolchain to be cached already. `-trimpath` allows compiled
generator packages to be reused across temporary working directories.

Generated files import `github.com/ton-blockchain/tolk-abi-to-go` (package
`tolkabi`). This runtime does not require CGO or Toncenter modules. Add it at a
compatible pinned version to the consuming Go project. Acton does not read or
modify the caller's `go.mod`, `go.sum`, or Go workspace to run its generator.
